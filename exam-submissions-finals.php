...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Finals Exam Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            My Students' Finals Exam Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Finals Period Exam Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                My Students' Finals Exam Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="finalsExamStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="completed">Completed</option>
                                    <option value="in_progress">In Progress</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testFinalsExamFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="finalsExamSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
                                        <th>Exam Title</th>
...
<!-- Custom JavaScript for Finals Exam Submissions -->
<script>
// Global variables
var finalsExamSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializeFinalsExamSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Finals Exam Submissions DataTable
 */
function initializeFinalsExamSubmissionsTable() {
    finalsExamSubmissionsTable = $("#finalsExamSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('finalsExamStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Finals Exam Submissions DataTable - Sending filters:', {
        ...
        "responsive": true,
        "language": {
            "processing": "Loading finals exam submissions...",
            "emptyTable": "No finals exam submissions found",
            "zeroRecords": "No matching finals exam submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Finals Exam Submissions - Status Filter Changed:', selectedValue);
    
    if (finalsExamSubmissionsTable) {
        finalsExamSubmissionsTable.ajax.reload();
    }
}

/**
 * Test finals exam filters function
 */
function testFinalsExamFilters() {
    console.log('=== FINALS EXAM SUBMISSIONS FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiExamSubmissions.php?action=datatable&draw=999&start=0&length=10&grading_period=finals&status=${status}`;
