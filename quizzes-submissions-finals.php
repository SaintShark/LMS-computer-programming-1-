...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Finals Quiz Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            My Students' Finals Quiz Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Finals Period</li>
            </ol>
        </nav>
    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Finals Period Quiz Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                My Students' Finals Quiz Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="finalsQuizStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="submitted">Submitted</option>
                                    <option value="in_progress">In Progress</option>
                                    <option value="timeout">Timeout</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testFinalsQuizFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="finalsQuizSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
                                        <th>Quiz Title</th>
...
<script>
    ...
    window.gradingPeriod = 'finals'; // Set the grading period for this page
</script>

<!-- Custom JavaScript for Finals Quiz Submissions -->
<script>
// Global variables
var finalsQuizSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializeFinalsQuizSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Finals Quiz Submissions DataTable
 */
function initializeFinalsQuizSubmissionsTable() {
    finalsQuizSubmissionsTable = $("#finalsQuizSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('finalsQuizStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Finals Quiz Submissions DataTable - Sending filters:', {
                    grading_period: 'finals',
                    status: status
                });
                
                // Always filter by finals period
                d.grading_period = 'finals';
        ...
        "responsive": true,
        "language": {
            "processing": "Loading finals quiz submissions...",
            "emptyTable": "No finals quiz submissions found",
            "zeroRecords": "No matching finals quiz submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Finals Quiz Submissions - Status Filter Changed:', selectedValue);
    
    if (finalsQuizSubmissionsTable) {
        finalsQuizSubmissionsTable.ajax.reload();
    }
}

/**
 * Test finals quiz filters function
 */
function testFinalsQuizFilters() {
    console.log('=== FINALS QUIZ SUBMISSIONS FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiQuizSubmissions.php?action=datatable&draw=999&start=0&length=10&grading_period=finals&status=${status}`;
