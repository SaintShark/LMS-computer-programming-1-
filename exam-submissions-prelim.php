...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Prelim Exam Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            My Students' Prelim Exam Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Prelim Period Exam Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                My Students' Prelim Exam Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="prelimExamStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="completed">Completed</option>
                                    <option value="in_progress">In Progress</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testPrelimExamFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="prelimExamSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
                                        <th>Exam Title</th>
...
<!-- Custom JavaScript for Prelim Exam Submissions -->
<script>
// Global variables
var prelimExamSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializePrelimExamSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Prelim Exam Submissions DataTable
 */
function initializePrelimExamSubmissionsTable() {
    prelimExamSubmissionsTable = $("#prelimExamSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('prelimExamStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Prelim Exam Submissions DataTable - Sending filters:', {
        ...
        "responsive": true,
        "language": {
            "processing": "Loading prelim exam submissions...",
            "emptyTable": "No prelim exam submissions found",
            "zeroRecords": "No matching prelim exam submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Prelim Exam Submissions - Status Filter Changed:', selectedValue);
    
    if (prelimExamSubmissionsTable) {
        prelimExamSubmissionsTable.ajax.reload();
    }
}

/**
 * Test prelim exam filters function
 */
function testPrelimExamFilters() {
    console.log('=== PRELIM EXAM SUBMISSIONS FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiExamSubmissions.php?action=datatable&draw=999&start=0&length=10&grading_period=prelim&status=${status}`;
