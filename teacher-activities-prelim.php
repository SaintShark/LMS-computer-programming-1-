...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Prelim Student Activity Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            Prelim - My Students' Activity Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Prelim Period - All Student Activity Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                Prelim Period - My Students' Activity Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="prelimStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="submitted">Submitted</option>
                                    <option value="unsubmitted">Unsubmitted</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testPrelimFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="prelimSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
...
<!-- Custom JavaScript for Prelim Activities -->
<script>
// Global variables
var prelimSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializePrelimSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Prelim Submissions DataTable
 */
function initializePrelimSubmissionsTable() {
    prelimSubmissionsTable = $("#prelimSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('prelimStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Prelim Activities DataTable - Sending filters:', {
        ...
        "responsive": true,
        "language": {
            "processing": "Loading prelim submissions...",
            "emptyTable": "No prelim submissions found",
            "zeroRecords": "No matching prelim submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Prelim Activities - Status Filter Changed:', selectedValue);
    
    if (prelimSubmissionsTable) {
        prelimSubmissionsTable.ajax.reload();
    }
}

/**
 * Test prelim filters function
 */
function testPrelimFilters() {
    console.log('=== PRELIM ACTIVITIES FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiTeacherActivities.php?action=datatable&draw=999&start=0&length=10&grading_period=prelim&status=${status}`;
