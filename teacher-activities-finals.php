...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Finals Student Activity Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            Finals - My Students' Activity Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Finals</li>
            </ol>
        </nav>
    </div>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Finals Period - All Student Activity Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                Finals Period - My Students' Activity Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="finalsStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="submitted">Submitted</option>
                                    <option value="unsubmitted">Unsubmitted</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testFinalsFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="finalsSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
...
<!-- Custom JavaScript for Finals Activities -->
<script>
// Global variables
var finalsSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializeFinalsSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Finals Submissions DataTable
 */
function initializeFinalsSubmissionsTable() {
    finalsSubmissionsTable = $("#finalsSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('finalsStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Finals Activities DataTable - Sending filters:', {
        ...
        "responsive": true,
        "language": {
            "processing": "Loading finals submissions...",
            "emptyTable": "No finals submissions found",
            "zeroRecords": "No matching finals submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Finals Activities - Status Filter Changed:', selectedValue);
    
    if (finalsSubmissionsTable) {
        finalsSubmissionsTable.ajax.reload();
    }
}

/**
 * Test finals filters function
 */
function testFinalsFilters() {
    console.log('=== FINALS ACTIVITIES FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiTeacherActivities.php?action=datatable&draw=999&start=0&length=10&grading_period=finals&status=${status}`;
