...
<main id="main" class="main">
    <div class="pagetitle">
        <h1>
            ...
            Midterm Quiz Submissions Management
            <?php elseif (Permission::isTeacher()): ?>
            My Students' Midterm Quiz Submissions
            <?php endif; ?>
        </h1>
        <nav>
            <ol class="breadcrumb">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h5 class="card-title">
                                <?php if (Permission::isAdmin()): ?>
                                Midterm Period Quiz Submissions
                                <?php elseif (Permission::isTeacher()): ?>
                                My Students' Midterm Quiz Submissions
                                <?php endif; ?>
                            </h5>
                            <div class="d-flex gap-2 align-items-center">
                                <select class="form-select" id="midtermQuizStatusFilter" style="width: auto;" onchange="filterByStatus()">
                                    <option value="">All Status</option>
                                    <option value="submitted">Submitted</option>
                                    <option value="in_progress">In Progress</option>
                                    <option value="timeout">Timeout</option>
                                </select>
                                <button type="button" class="btn btn-outline-info" onclick="testMidtermQuizFilters()" title="Test Filters">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped" id="midtermQuizSubmissionsTable">
                                <thead>
                                    <tr>
                                        <th>Student Name</th>
                                        <th>Quiz Title</th>
...
<!-- Custom JavaScript for Midterm Quiz Submissions -->
<script>
// Global variables
var midtermQuizSubmissionsTable;
var currentSubmissionId = null;

$(document).ready(function () {
    initializeMidtermQuizSubmissionsTable();
    setupModalEvents();
});

/**
 * Initialize Midterm Quiz Submissions DataTable
 */
function initializeMidtermQuizSubmissionsTable() {
    midtermQuizSubmissionsTable = $("#midtermQuizSubmissionsTable").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            type: "GET",
            data: function(d) {
                const statusEl = document.getElementById('midtermQuizStatusFilter');
                const status = statusEl ? statusEl.value : '';
                
                console.log('Midterm Quiz Submissions DataTable - Sending filters:', {
        ...
        "responsive": true,
        "language": {
            "processing": "Loading midterm quiz submissions...",
            "emptyTable": "No midterm quiz submissions found",
            "zeroRecords": "No matching midterm quiz submissions found"
        }
    });
}
...
function filterByStatus() {
    ...
    console.log('Midterm Quiz Submissions - Status Filter Changed:', selectedValue);
    
    if (midtermQuizSubmissionsTable) {
        midtermQuizSubmissionsTable.ajax.reload();
    }
}

/**
 * Test midterm quiz filters function
 */
function testMidtermQuizFilters() {
    console.log('=== MIDTERM QUIZ SUBMISSIONS FILTER DEBUG ===');
    ...
    // Test API directly
    const testUrl = `app/API/apiQuizSubmissions.php?action=datatable&draw=999&start=0&length=10&grading_period=midterm&status=${status}`;
